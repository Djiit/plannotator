...
| `PLANNOTATOR_ORIGIN` | auto-detect | Explicit agent-origin override. Valid values: `claude-code`, `amp`, `droid`, `opencode`, `codex`, `copilot-cli`, `pi`, `oh-my-pi`, `gemini-cli`, `kiro-cli`, `mistral-vibe`. Invalid values silently fall through to env-based detection. |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `PLANNOTATOR_GIT_REMOTE_CHECK` | enabled | Set to `0`, `false`, or `disabled` to stop local Git code review contacting the remote. By default a review runs `git ls-remote --symref origin HEAD` to find the remote default branch and to tell whether your baseline is behind it ("Baseline is behind GitHub"). It runs when the review opens, when the diff loads, when you switch diff type or base (including the "Diff out of date · Refresh" button), and when you press Fetch — at most once a minute, never on a timer, so an idle review page makes no requests. Turn it off when a remote probe is expensive or intrusive, most sharply when SSH authentication is backed by a hardware token: with the check off no `ls-remote` runs at all, the compare target comes from your local refs, and the "behind GitHub" banner never appears — and with it the one-click Fetch, since that button lives in the banner (the endpoint itself stays available). Fetching from your own terminal is unaffected. Can also be set via `~/.plannotator/config.json` (`{ "gitRemoteCheck": false }`) or per-invocation via `plannotator review --no-git-remote-check`; the flag beats the env var, which beats the config key. |
